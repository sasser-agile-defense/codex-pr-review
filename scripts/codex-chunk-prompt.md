...
- If you notice something that depends on code in a different chunk (e.g., a function defined elsewhere, a type imported from a file not shown), flag it as a **priority 0 (info)** observation noting the cross-chunk uncertainty.
...
4. **Prioritize severity** — Use priority 3 (high) for bugs and security issues. Use priority 2 (medium) for performance and important maintainability issues. Use priority 1 (low) for minor improvements. Use priority 0 (info) for observations and cross-chunk uncertainty.
